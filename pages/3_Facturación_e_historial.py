...
# ==========================================
_mostrar_animacion_pagina = st.session_state.get("_ultima_pagina_animada") != "facturacion_historial"
st.session_state["_ultima_pagina_animada"] = "facturacion_historial"
_anim_pagina_css = (
...
with tab_cotizaciones:
    ...
    st.caption(
        "Cotizaciones creadas desde Recepción → pestaña Cotización. Descárgalas en PDF o "
        "conviértelas en una orden real asignando los mecánicos responsables."
    )

    cotizaciones = obtener_cotizaciones(user_id)

    if not cotizaciones:
        st.info("Aún no se ha guardado ninguna cotización. Créalas desde **Recepción → pestaña Cotización**.")
    else:
        pendientes = [c for c in cotizaciones if not c.convertida_a_hoja_id]
        convertidas = [c for c in cotizaciones if c.convertida_a_hoja_id]
        st.markdown(f"**{len(pendientes)}** pendiente(s) de convertir · **{len(convertidas)}** ya convertida(s) en orden real.")
        st.markdown("---")

        for cot in cotizaciones:
            vehiculo_txt = cot.placa
            if cot.marca or cot.modelo:
                vehiculo_txt += f" — {' '.join(v for v in (cot.marca, cot.modelo) if v)}"

            with st.container(border=True):
                col_h1, col_h2, col_h3 = st.columns([3, 2, 2])
                with col_h1:
                    st.markdown(f"**Cotización #{cot.numero_cotizacion}** — {vehiculo_txt}")
                    st.caption(f"Cliente: {cot.razon_social}")
                with col_h2:
                ...
                with st.expander("Ver detalle, descargar PDF o convertir a orden"):
                    encabezado, df_items_cot = obtener_cotizacion_detalle(user_id, cot.id)

                    if df_items_cot is None or df_items_cot.empty:
                        st.info("Esta cotización no tiene ítems.")
                    else:
                        df_mostrar_cot = df_items_cot[['tipo_item', 'descripcion', 'precio_venta', 'iva_tipo']].copy()
                        df_mostrar_cot['iva_tipo'] = df_mostrar_cot['iva_tipo'].fillna('(default de categoría)')
                        df_mostrar_cot.columns = ['Tipo', 'Descripción', 'Precio Estimado', 'Impuesto (IVA)']
                        st.dataframe(
                            df_mostrar_cot.style.format({'Precio Estimado': lambda x: formato_cop(x)}),
                            width='stretch', hide_index=True
                        )

                        subtotal_cot, iva_valor_cot, gran_total_cot, desglose_iva_cot = calcular_totales_orden(
                            df_items_cot, iva_activo=IVA_ACTIVO, iva_incluido=IVA_INCLUIDO,
                            iva_tipo_default_mano_obra=IVA_TIPO_DEFAULT_MO, iva_tipo_default_repuestos=IVA_TIPO_DEFAULT_REP
                        )
                        st.success(f"Total estimado: {formato_cop(gran_total_cot)}")

                        st.markdown("---")
                        st.markdown("#### Descargar PDF")

                        logo_path_cot = _config_taller[3] if _config_taller and _config_taller[3] else None
                        taller_nit_cot = _config_taller[8] if _config_taller and len(_config_taller) > 8 and _config_taller[8] else ""
                        taller_telefono_cot = _config_taller[9] if _config_taller and len(_config_taller) > 9 and _config_taller[9] else ""
                        taller_direccion_raw_cot = _config_taller[10] if _config_taller and len(_config_taller) > 10 and _config_taller[10] else ""
                        taller_ciudad_cot = _config_taller[11] if _config_taller and len(_config_taller) > 11 and _config_taller[11] else ""
                        taller_direccion_cot = f"{taller_direccion_raw_cot}, {taller_ciudad_cot}".strip(", ") if taller_ciudad_cot else taller_direccion_raw_cot
                        taller_email_cot = _config_taller[2] if _config_taller and _config_taller[2] else ""

                        pdf_bytes_cot = generar_pdf_orden_profesional(
                            taller_nombre=nombre_taller,
                            taller_nit=taller_nit_cot,
                            taller_telefono=taller_telefono_cot,
                            taller_direccion=taller_direccion_cot,
                            taller_email=taller_email_cot,
                            taller_logo_path=logo_path_cot,
                            hoja_id=encabezado.numero_cotizacion,
                            fecha=str(encabezado.fecha_creacion)[:10],
                            cliente=encabezado.razon_social,
                            cliente_nit=encabezado.nit,
                            placa=encabezado.placa,
                            estado="Cotización",
                            df_items=df_items_cot,
                            total=gran_total_cot,
                            subtotal=subtotal_cot if IVA_ACTIVO else None,
                            desglose_iva=desglose_iva_cot if IVA_ACTIVO else None,
                            marca=encabezado.marca or "",
                            modelo=encabezado.modelo or "",
                            es_cotizacion=True,
                        )
                        ...
                        st.download_button(
                            ...
                            mime="application/pdf",
                            key=f"pdf_cot_{cot.id}",
                            width='stretch'
                        )

                        if not cot.convertida_a_hoja_id:
                            st.markdown("---")
                            st.markdown("#### Convertir en Orden Real")
                            st.caption("Asigna un mecánico responsable a cada ítem de mano de obra para poder convertirla.")

                            items_mo_cot = df_items_cot[df_items_cot['tipo_item'] == 'Mano de Obra']

                            if not dict_mecanicos:
                                st.warning("Este taller no tiene mecánicos registrados todavía. Regístralos en Registros antes de convertir esta cotización.")
                            else:
                                with st.form(key=f"form_convertir_{cot.id}"):
                                    mecanicos_sel_cot = {}
                                    for _, row_mo in items_mo_cot.iterrows():
                                        mec_sel_item = st.selectbox(
                                            f"Mecánico para: {row_mo['descripcion']}",
                                            options=list(dict_mecanicos.keys()),
                                            key=f"mec_convertir_{cot.id}_{row_mo['id']}"
                                        )
                                        mecanicos_sel_cot[int(row_mo['id'])] = dict_mecanicos[mec_sel_item]

                                    estados_convertir = ["Cotizar", "En revisión", "Esperando repuestos", "En reparación", "Listo para facturar"]
                                    estado_convertir_sel = st.selectbox("Estado inicial de la orden", estados_convertir, index=1)

                                    convertir_click = st.form_submit_button("Convertir en Orden Real", type="primary", width='stretch')

                                if convertir_click:
                                    try:
                                        numero_orden_nueva = convertir_cotizacion_a_orden(
                                            user_id, cot.id, estado_convertir_sel, mecanicos_sel_cot
                                        )
                                        st.success(f"Cotización convertida en la Orden #{numero_orden_nueva}. Ya puedes gestionarla en el Historial de Órdenes.")
                                        st.rerun()
                                    except ValueError as e:
                                        st.error(str(e))
                                    except Exception as e:
                            ...
                            if st.session_state.get(f"confirm_del_cot_{cot.id}", False):
                                col_dc1, col_dc2, col_dc3 = st.columns([2, 1, 1])
                                col_dc1.warning("¿Eliminar esta cotización? Esta acción no se puede deshacer.")
                                with col_dc2:
                                    if st.button("Sí, eliminar", key=f"yes_del_cot_{cot.id}", type="primary", width='stretch'):
                                        try:
                                            eliminar_cotizacion(user_id, cot.id)
                                            st.session_state[f"confirm_del_cot_{cot.id}"] = False
                                            st.success("Cotización eliminada.")
                                            st.rerun()
                                        except Exception as e:
                                            st.error(mensaje_error_amigable(e, "eliminar la cotización"))
                                with col_dc3:
                                    if st.button("Cancelar", key=f"no_del_cot_{cot.id}", width='stretch'):
                                        st.session_state[f"confirm_del_cot_{cot.id}"] = False
                                        st.rerun()
